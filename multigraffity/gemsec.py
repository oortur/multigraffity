...
from .walkers import RandomWalker, MultiRandomWalker, MultiRandomWalkerWithHops
...
class MultiGEMSEC(Estimator):
    """
    Modification of GEMSEC method for Multigraph. Uses sampling strategy based on
    weighted MultiRandomWalk or MultiRandomWalkWithHops.
    Args:
        walk_number (int): Number of random walks. Default is 5.
        walk_length (int): Length of random walks. Default is 80.
        dimensions (int): Dimensionality of embedding. Default is 32.
        negative_samples (int): Number of negative samples. Default is 5.
        window_size (int): Matrix power order. Default is 5.
        hop_rate (float): Rate of link type change while walking. Default is 1.
        learning_rate (float): Gradient descent learning rate. Default is 0.1.
        clusters (int): Number of cluster centers. Default is 10.
        gamma (float): Clustering cost weight coefficient. Default is 0.1.
        seed (int): Random seed value. Default is 42.
    """
    def __init__(self, walk_number: int=5, walk_length: int=80, dimensions: int=32,
                 negative_samples: int=5, window_size: int=5, hop_rate: float=1,
                 learning_rate: float=0.1, clusters: int=10, gamma: float=0.1, seed: int=42):
    ...
    def fit(self, graph: nx.classes.multigraph.MultiGraph):
        """
        Fitting a MultiGEMSEC model.
        Arg types:
            * **graph** *(NetworkX Multigraph)* - The graph to be embedded.
        """
        self._set_seed()
        self._check_graph(graph)
        self._setup_sampling_weights(graph)
        if self.hop_rate == 1:
            self._walker = MultiRandomWalker(self.walk_length, self.walk_number)
        else:
            self._walker = MultiRandomWalkerWithHops(self.walk_length, self.walk_number, self.hop_rate)
        self._walker.do_walks(graph)
        self._initialize_node_embeddings(graph)
        self._initialize_cluster_centers(graph)
        self._do_gradient_descent()
